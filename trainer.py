...
class Trainer:
    ...
    def train(self, dataloader: DataLoader, epochs: int, device: str = "", grid_num_images = 64, do_display = True):
        ...
        self._setup_fig(grid_num_images)
    ...
    def maybe_print_status(self, epoch: int, epochs: int, global_step: int, batch: int,
        ...
        if delta_last >= datetime.timedelta(seconds=10) or epoch == epochs:
            ...
            data_idx = batch * self._batch_size
            ...
            if self._do_display:
                self.show_images(epochs, fake_images, real_images)

            basename = f"{self.dirname}/epoch{epoch:05}-step{global_step:06}"

            print(f"epoch {epoch + 1}/{epochs}, data {data_idx}/{self._num_data} | loss {loss:.4f} | {persec_first:.4f} samples/sec")
    ...
    def _setup_fig(self, grid_num_images: int):
        ...
        self._grid_num_images = grid_num_images
        self._grid_rows = int(np.sqrt(grid_num_images))

    def show_images(self, epoch: int, real_images: torch.Tensor, fake_images: torch.Tensor):
