...
# NOTE transferred verbatim from SD
class Upsample(nn.Module):
    def __init__(self, in_channels: bool, with_conv: bool):
        super().__init__()
    ...
    def extra_repr(self) -> str:
        return f"in_channels={self.in_channels}, with_conv={self.with_conv}"
...
class ResnetBlock(nn.Module):
    ...
    def extra_repr(self) -> str:
        fields = list()
        for k in "in_channels out_channels use_conv_shortcut".split(" "):
            fields.append((k, str(getattr(self, k))))

        return ",".join([f"{k}={v}" for k, v in fields])

# NOTE all attention stuff removed.
class Model(nn.Module):
    def __init__(self, *, 
                 ch: int,               # NOTE mid_channels?
                 out_ch: int,           # NOTE out_channels?
                 ch_mult=(1, 2, 4, 8),  #
                 num_res_blocks: int,
                 #attn_resolutions, 
                 dropout = 0.0, 
    ...
    def extra_repr(self) -> str:
        fields = list()
        for k in "ch out_ch ch_mult num_res_blocks resamp_with_conv in_channels resolution".split(" "):
            fields.append((k, str(getattr(self, k))))
