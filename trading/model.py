...
def simulate(net: nn.Module, val_dataloader: DataLoader, num_predictions: int = 0, print = 0) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    with torch.no_grad():
        ...
        if num_predictions == 0:
            num_predictions = num_quotes
        for i in range(num_predictions):
            out = net(inputs)
            if print and num_predictions < print:
    ...
    # return the size we were asked for, and return a view as 1d (num, ), instead of 2d (num, 1)
    actual_quotes = actual_quotes[:num_predictions].view((num_predictions,))
    pred_quotes = pred_quotes[:num_predictions].view((num_predictions,))
