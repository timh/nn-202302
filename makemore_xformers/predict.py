...
parser.add_argument("--temp", "--temperature", default=1.0, type=float)
...
while True:
    s = model_utils.predict(net=exp.net, seq_len=exp.seqlen, tokenizer=exp.tokenizer, dictionary=exp.dictionary, 
                            num_preds=cfg.pred_len, 
                            start_text=start_text, top_k=cfg.top_k, temperature=cfg.temp,
                            device="cuda")
